...
def get_public_name(user: User | None) -> str:
    if user is None:
        return "неизвестно"
    if user.username:
...
def format_request(request: HelpRequest, owner: User | None = None) -> str:
    ...
    return (
        ...
        f"Оплата: {reward}\n"
        f"Автор: {get_public_name(request_owner)}\n\n"
...
async def nearby_requests_handler(message: Message) -> None:
    async with SessionFactory() as session:
        ...
        prepared = [(request, format_request(request)) for request in requests]
    if not prepared:
        await message.answer("Пока нет опубликованных заявок рядом.")
        return
    for request, text in prepared:
...
async def my_requests_handler(message: Message) -> None:
    async with SessionFactory() as session:
        ...
        prepared = [format_request(request, owner=user) for request in requests]
    if not prepared:
...
async def offer_save(message: Message, state: FSMContext) -> None:
    ...
    await state.clear()
    await message.answer("Отклик сохранен.", reply_markup=MAIN_MENU)
