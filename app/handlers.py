...
from app.keyboards import CATEGORIES, MAIN_MENU, REWARD_TYPES, categories_keyboard, moderation_keyboard, request_actions_keyboard, reward_keyboard
from app.models import HelpRequest, HelpRequestStatus, User
from app.repositories import create_help_request, create_offer, get_or_create_user, get_request_by_id, list_published_requests, list_user_requests, set_request_status, update_user_location
from app.states import ComplaintState, CreateRequestState, OfferState, ProfileState
...
def request_text(request: HelpRequest) -> str:
    ...
    owner = user_name(request.owner) if request.owner else "неизвестно"
    return (
...
async def current_user(message: Message, session: AsyncSession) -> User:
...
async def current_user_from_callback(callback: CallbackQuery, session: AsyncSession) -> User:
    return await get_or_create_user(
        session=session,
...
@router.message(F.text == "Правила безопасности")
async def safety_handler(message: Message) -> None:
    await message.answer(
        ...
        "5. Если заявка подозрительная, отправь жалобу."
    )
...
async def profile_handler(message: Message) -> None:
    ...
    await message.answer("Например: <code>Алматы, Бостандыкский</code>")
    await message.answer("Отправь город и район одним сообщением или нажми /cancel.")
...
async def save_profile_location(message: Message, state: FSMContext) -> None:
    if not message.text:
        await message.answer("Отправь текстом: город, район")
        return
    city, _, district = message.text.partition(",")
    city = city.strip()
    district = district.strip() or None
    if len(city) < 2:
        await message.answer("Город слишком короткий. Пример: Алматы, Бостандыкский")
        return
    async with SessionFactory() as session:
        user = await current_user(message, session)
...
@router.callback_query(CreateRequestState.category, F.data.startswith("category:"))
async def create_request_category(callback: CallbackQuery, state: FSMContext) -> None:
    category = callback.data.split(":", 1)[1]
    await state.update_data(category=category)
    await callback.message.answer("Короткий заголовок заявки. Например: Нужна помощь с лекарствами")
    await state.set_state(CreateRequestState.title)
    await callback.answer()


@router.message(CreateRequestState.title)
...
async def create_request_description(message: Message, state: FSMContext) -> None:
    ...
    await message.answer("Укажи город. Например: Алматы")
    await state.set_state(CreateRequestState.city)


@router.message(CreateRequestState.city)
async def create_request_city(message: Message, state: FSMContext) -> None:
    ...
    await message.answer("Укажи район. Например: Бостандыкский. Если не важно, напиши -")
    await state.set_state(CreateRequestState.district)


@router.message(CreateRequestState.district)
...
@router.message(CreateRequestState.address_hint)
async def create_request_address(message: Message, state: FSMContext) -> None:
    ...
    await message.answer("Когда нужна помощь? Например: сегодня до 18:00")
    await state.set_state(CreateRequestState.needed_at_text)


@router.message(CreateRequestState.needed_at_text)
async def create_request_needed_at(message: Message, state: FSMContext) -> None:
    value = (message.text or "").strip()
    await state.update_data(needed_at_text=value)
...
@router.callback_query(CreateRequestState.reward_type, F.data.startswith("reward:"))
async def create_request_reward(callback: CallbackQuery, state: FSMContext) -> None:
    reward_type = callback.data.split(":", 1)[1]
    await state.update_data(reward_type=reward_type)
    if reward_type == "paid":
        await callback.message.answer("Укажи сумму числом. Например: 3000")
...
@router.message(CreateRequestState.reward_amount)
...
async def create_request_confirm(message: Message, state: FSMContext) -> None:
    ...
    await message.answer(request_text(request))


@router.message(F.text.in_({"Заявки рядом", "Хочу помочь"}))
async def nearby_requests_handler(message: Message) -> None:
    async with SessionFactory() as session:
        user = await current_user(message, session)
        requests = await list_published_requests(session, city=user.city, district=user.district, limit=10)
        if not requests:
            requests = await list_published_requests(session, city=user.city, district=None, limit=10)
    ...
    for request in requests:
        await message.answer(
            ...
            reply_markup=request_actions_keyboard(request.id, request.owner.telegram_id, message.from_user.id),
        )
...
@router.message(OfferState.message)
async def offer_save(message: Message, state: FSMContext) -> None:
    ...
    await state.clear()
    await message.answer("Отклик отправлен автору заявки.", reply_markup=MAIN_MENU)


@router.callback_query(F.data.startswith("mod_publish:"))
async def moderator_publish(callback: CallbackQuery) -> None:
    ...
    async with SessionFactory() as session:
        ...
        if request is None:
            ...
            return
        await set_request_status(session, request, HelpRequestStatus.PUBLISHED)
    await callback.message.answer("Заявка опубликована.")
    await callback.answer()


@router.callback_query(F.data.startswith("mod_reject:"))
async def moderator_reject(callback: CallbackQuery) -> None:
    if not settings.is_admin(callback.from_user.id):
        await callback.answer("Нет доступа", show_alert=True)
        return
    request_id = int(callback.data.split(":", 1)[1])
    async with SessionFactory() as session:
        ...
        if request is None:
            await callback.answer("Заявка не найдена", show_alert=True)
            return
        await set_request_status(session, request, HelpRequestStatus.REJECTED)
